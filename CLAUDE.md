...
- **Homepage conversion pass (2026-09-18):** ✅ Hormozi-style audit fixes: one CTA label site-wide ("Get My Price", `components/quote/QuoteCta.tsx`) that scrolls to the on-page quote assistant; homepage links cut 47 → 28 with no exits in the page body; hero offer stack + client-computed "Next opening" (`HeroOffer`); compact 5-up services (swipe row on phones, no page exits); objections section (`Objections`, driven by `business.objections`, only `confirmed: true` entries render) replaces the duplicate reviews block; chat launcher and "ask the concierge" buttons hidden until `ANTHROPIC_API_KEY` is set (`.chat-only`); slim footer. Home is ~4,830px at 1440.
- **Distraction cut + service area (2026-09-18):** ✅ header menu cut to Services + Reviews (About, Service Area, FAQ, Contact now footer-only via `footerNav`); homepage `ServiceArea` section with a zip checker (uses `pricing.serviceZipPrefixes`) and a SAMPLE neighborhood list (`business.neighborhoods`, flip `neighborhoodsConfirmed` once the owner supplies real coverage); homepage body extracted to `HomeLanding`; `/get-quote` ad landing page (noindex, no nav, logo not linked, see `landingPaths`).
- **Mobile + booking pass (2026-09-18):** ✅ phone-first fixes on every page: the first quote tap now sits above the fold (≈503–535px on a 740px screen, was 680–740 under the sticky bar); the sticky mobile bar hides while a quote form is on screen; tighter hero/type/padding on phones; compact steps, swipe-row featured reviews, 44px footer tap targets, 11px min consent text. Quote assistant: step counter and "Sample pricing" badge removed; Studio chip fixed (one wider column); **address step added** (price → "Yes, book it" → address → booked → card), address flows to the lead/VA alert. Homepage: Google Reviews banner removed from the hero; service-area section is now a **real map** (MapLibre GL v5 + OpenFreeMap "dark" tiles recolored to the brand, no API key; lazy-loaded near the viewport) with a drawn-in coverage outline, drop-in pins, auto tour, and fly-to on tap; review marquee shows names only; Commercial card photo wired in; card icon badges removed.
